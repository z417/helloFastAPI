...
async function loadMyTickets() {
    ...
    try {
        ...
        tickets.forEach(ticket => {
            const dateStr = ticket.start_time.split('T')[0]; // "YYYY-MM-DD"
            if (!groups[dateStr]) {
                groups[dateStr] = [];
            }
        ...
        sortedDates.forEach(dateStr => {
            ...
            ticketsInDate.forEach(ticket => {
                ...
                if (ticket.status === 2) {
                    ...
                    actionHtml = `<span class="text-secondary small" style="font-size: 11px;">已退订退款</span>`;
                } else if (isExpired) {
                    // 已过期
        ...
        container.innerHTML = `<div class="col-12 text-center text-danger py-3 small">加载购票记录失败: ${error.message}</div>`;
    }
}
