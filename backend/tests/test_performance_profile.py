...
async def test_concurrency_and_performance_switches_comparison():
    ...
    async with httpx.AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as client:
        ...
        await client.post(
            "/api/cinema/config",
            json={"pool_mode": "queue", "lock_mode": "none", "slow_query": False, "signature_check": False},
        ...
        print(" ▸ 正在发起高并发抢购 [座位A]: 并发 20 个协程同时抢占无锁座位...")
        ...
        await client.post(
            "/api/cinema/config",
            json={"pool_mode": "queue", "lock_mode": "pessimistic", "slow_query": False, "signature_check": False},
            headers={"Authorization": f"Bearer {admin_token}"},
        )

        print(" ▸ 正在发起高并发抢购 [座位B]: 并发 20 个协程同时抢占悲观锁座位...")
        tasks_pess = [
        ...
        await client.post(
            "/api/cinema/config",
            json={"pool_mode": "queue", "lock_mode": "optimistic", "slow_query": False, "signature_check": False},
            headers={"Authorization": f"Bearer {admin_token}"},
        )

        print(" ▸ 正在发起高并发抢购 [座位C]: 并发 20 个协程同时抢占乐观锁座位...")
        tasks_opt = [
        ...
        tasks_pool_on = [client.get("/api/cinema/showtimes", headers={"Authorization": f"Bearer {user_token}"}) for _ in range(20)]
        ...
        await client.post(
            "/api/cinema/config",
            json={"pool_mode": "null", "lock_mode": "optimistic", "slow_query": False, "signature_check": False},
            headers={"Authorization": f"Bearer {admin_token}"},
        )
        ...
        t0 = time.perf_counter()
        tasks_pool_off = [client.get("/api/cinema/showtimes", headers={"Authorization": f"Bearer {user_token}"}) for _ in range(20)]
        ...
        await client.post(
            "/api/cinema/config",
            json={"pool_mode": "queue", "lock_mode": "optimistic", "slow_query": True, "signature_check": False},
            headers={"Authorization": f"Bearer {admin_token}"},
        )
        ...
        await client.post(
            "/api/cinema/config",
            json={"pool_mode": original_pool_mode, "lock_mode": "pessimistic", "slow_query": False, "signature_check": False},
            headers={"Authorization": f"Bearer {admin_token}"},
        )
