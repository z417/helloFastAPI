    ...
    async with httpx.AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as client:
        ...
        try:
            ...
            r_order2 = await client.post(
                "/api/cinema/order", json={"showtime_id": showtime_id, "seat_id": seat2_id, "signature": signature2}, headers=headers_replay
            )
...
async def test_sm3_signature_verification():
    async with httpx.AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as client:
        ...
        try:
            ...
            headers_sm3 = {"Authorization": f"Bearer {user_token}", "X-Timestamp": timestamp, "X-Nonce": nonce, "X-Signature": sig_sm3}
