...
class IPRATELimit:
    ...
    async def remove_expired_ip(self):
        ...
        expired_ips = [ip for ip, info in self.ips.items() if info["time"] + timedelta(minutes=self.minutes) < now]
    ...
    def __call__(self, request: Request):
        ...
        if not self.check_ip(ip):
            raise HTTPException(status_code=status.HTTP_429_TOO_MANY_REQUESTS, detail="Too many requests, do it later")
